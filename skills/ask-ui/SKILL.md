...
6. 用户提交后，解析写入 stdout 的 JSON 结果（多行缩进格式，整体读完再解析，不要逐行解析），立即继续原来的工作流。返回格式示例：
...
- 一个任务对应一个 Session，每批问题对应一个 Round，同一任务的所有轮次复用同一个 `sessionId`；仅在新任务、已完成的任务或用户明确要求重新开始时创建新 Session。
- 修正和补充确认一律开新 Round；已提交的问题与答案不可改动（禁止行为见「反例与禁止操作」）。
