 ...
 *
 * 用法:
 *   node skill-stats.mjs [--out <path>] [--projects-dir <dir>]
 *     --out           输出 HTML 路径，默认 <系统临时目录>/skill-stats.html
 *     --projects-dir  会话日志目录，默认 ~/.claude/projects
 *
 * 口径（与用户逐条确认过，勿轻易改动）:
 *  - 模型调用: assistant 消息中 name === 'Skill' 的 tool_use，取 input.skill / input.args
 *  - 手动触发: user 消息正文为真实斜杠命令展开（以 <command-message> 开头、与
 *    <command-name> 成对），剔除内置命令（clear/model/mcp/compact/handoff/init）
 *  - MCP 调用: name 以 mcp__ 开头的 tool_use，只按 server 汇总，不按具体工具细分
 *  - 递归扫描全部 *.jsonl（含子代理轨迹 subagents/*.jsonl），
 *    按 tool_use.id / 行 uuid 去重，防镜像轨迹重复计数
...
function extractFile(file, records, seen) {
  let text;
  try {
    ...
    if (obj.type === 'assistant' && obj.message && Array.isArray(obj.message.content)) {
      for (const b of obj.message.content) {
        ...
        if (b.name === 'Skill') {
          ...
          records.push({ ts, cwd, proj, name: skill, ch: 'model', args });
          ...
          records.push({ ts, cwd, proj, name: server, ch: 'mcp', args: '' });
        } else {
          continue;
        }
      ...
      records.push({ ts, cwd, proj, name: cmd, ch: 'slash', args: am ? am[1].trim() : '' });
    }
  }
}
...
<header>
  <div>
    ...
    <div class="meta">数据快照 __GEN_DATE__ · 来源 ~/.claude/projects 会话 JSONL · 共 __COUNT__ 条 · 范围 __RANGE__</div>
  </div>
...
<section class="card">
  ...
  <table class="tbl">
    <thead><tr><th>时间</th><th>名称</th><th>渠道</th><th>项目</th><th>参数摘要</th></tr></thead>
    <tbody id="tbody"></tbody>
  </table>
</section>

<footer class="foot">
  口径：「模型调用」= 会话中模型主动发起的 Skill 工具调用；「手动触发」= 用户输入的 /斜杠命令（仅计真实命令展开，已剔除 clear、model、mcp、compact、handoff、init 等内置命令）；「MCP 调用」= 模型发起的 mcp__&lt;server&gt;__&lt;tool&gt; 工具调用，只按 server 汇总，不按具体工具细分。扫描范围含子代理轨迹，每条调用按唯一 ID 去重；统计含生成快照时正在进行的会话本身（自计数）。~/.claude/projects 只保留近期会话，非全量历史。
</footer>
...
var state = { from: '', to: '', project: '', name: '', ch: '' };
...
if (!fs.existsSync(opts.projectsDir)) {
  console.error('会话日志目录不存在: ' + opts.projectsDir);
  process.exit(1);
}

const files = listJsonl(opts.projectsDir);
const records = [];
const seen = new Set();
for (const f of files) extractFile(f, records, seen);
...
console.log('  会话文件 : ' + files.length + ' 个');
